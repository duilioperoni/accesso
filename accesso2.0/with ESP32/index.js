...
//Events the boards call when a presence is detected
app.get('/occupybath1', (req, res) => {
    res.send('bath 1 occupied');
    status = readStatus();
...
app.get('/occupybath2', (req, res) => {
    res.send('bath 2 occupied');
    status = readStatus();
...
//Events the boards call when a presence is no longer detected
app.get('/freebath1', (req, res) => {
    res.send('bath 1 in need of sanification');
    status = readStatus();
    status2 = String(status).charAt(2);
    status1 = 3;
    saveStatus(String(status1) + "," + String(status2));
...
app.get('/freebath2', (req, res) => {
    res.send('bath 2 in need of sanification');
    status = readStatus();
    status1 = String(status).charAt(0);
    status2 = 3;
    saveStatus(String(status1) + "," + String(status2));
...
//Events that can only be called from the janitor app to inform when a restroom is being sanificated
